...
GD<- function(XX,Xy,b=rep(0,ncol(XX)), RSS=1,nIter=10,learning_rate=1/5,lambda=0,b0=rep(0,ncol(XX)),lambda0=1,returnPath=FALSE){
    ...
    if(returnPath){
     ...
     B[,1]=b
     for(i in 2:ncol(B)){
        B[,i]=.Call("GRAD_DESC",XX, Xy, B[,i-1], 1, learning_rate)[[1]]
     }
