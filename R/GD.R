...
GD<- function(XX,Xy,p=ncol(XX),b=rep(0,p), RSS=1,nIter=10,learning_rate=1/5,lambda=0,b0=rep(0,p),lambda0=1,returnPath=FALSE){
    ...
    if(returnPath){
     ...
     for(i in 2:ncol(B)){
        ...
        B[,i]=.Call("GRAD_DESC",XX, Xy, B[,i-1],p, 1, learning_rate)[[1]]
     }
     rownames(B)=rownames(XX)
     colnames(B)=paste0('iter_',0:(nIter-1))
    }else{
         B <- .Call("GRAD_DESC",XX, Xy, b, p, nIter, learning_rate)[[1]]
    }
    return(B)
}
